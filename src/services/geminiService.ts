...
export const generateStoryElements = (topic: string, numScenes: number, style: string, characterGender: 'male' | 'female', hasCharacterImage: boolean): Promise<{ thumbnailPrompt: string, scenes: Omit<SceneCard, 'sceneNumber'>[] }> => {
  return callApi('generateStoryElements', { topic, numScenes, style, characterGender, hasCharacterImage });
};
export const generateImageForScene = (visualsPrompt: string, aspectRatio: string, characterImage?: string | null): Promise<string> => {
