...
import { SparklesIcon, PhotoIcon, ArrowDownTrayIcon, SettingsIcon, ChatBubbleBottomCenterTextIcon } from './Icons';
        ...
        <div className="aspect-square bg-gray-700/50 rounded-lg flex items-center justify-center p-4">
          {isLoadingImage ? (
            ...
            <div className="relative group w-full h-full">
                <img 
                    src={isOverlayVisible ? imageUrl : imageWithoutOverlay} 
                ...
                />
                 <div 
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') setIsOverlayVisible(prev => !prev); }}
                >
                    <div className="absolute inset-0 bg-black/70 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-md">
                        <div className="text-center text-white pointer-events-none">
                            <ChatBubbleBottomCenterTextIcon className="h-12 w-12 mx-auto" />
                            <p className="font-bold text-lg mt-2">Click to toggle text</p>
                        </div>
                    </div>
                </div>
                <a
                    href={isOverlayVisible ? imageUrl : imageWithoutOverlay}
                    download="motivational-image.png"
                    className="absolute top-3 right-3 p-2.5 bg-gray-900/60 hover:bg-gray-900/80 rounded-full text-white opacity-0 group-hover:opacity-100 transition-all duration-300 z-10"
